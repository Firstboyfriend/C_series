...
//    if (GetQueryString("IsP") != undefined && GetQueryString("IsP") == "NO") {
...
function doQuery() {
    ...
    var loccode = $("#locomotive_no").text(); //设备编码

    var url = '../Foundation/RemoteHandlers/LocomotiveControl.ashx?type=all' + '&LOCOMOTIVE_CODE=' + escape(loccode);
    var jwd, psd, date, jl, yyqd, cc, sjh;
    $.ajax({
        type: "POST",
        url: url,
        async: true,
        ...
        success: function (result) {
            json = eval('(' + result + ')');
            document.getElementById('locomotiveSect').innerHTML = json.rows[0].ORG_NAME;
            document.getElementById('useSector').innerHTML = json.rows[0].USE_DEPT_NAME;
        }
    });

    url = 'RemoteHandlers/VideoDeviceInfoHandler.ashx?type=getTodayRunningStatus&locomotiveCode=' + escape(loccode);
    $.ajax({
        type: "POST",
        url: url,
        async: true,
        ...
        success: function (result) {
            json = eval('(' + result + ')');
            if (json.length > 0) {
                //                document.getElementById('last_running_time').innerHTML = dateFormat(ConvertJSONDateToJSDateObject(json[0].DETECT_TIME),'yyyy-MM-dd');
                var datearray = json[0].DETECT_TIME.split(' ');
                document.getElementById('last_running_time').innerHTML = datearray[0];
                document.getElementById('routing_no').innerHTML = json[0].CROSSING_NO;
                document.getElementById('running_section').innerHTML = json[0].AREA_SECTION;
                document.getElementById('train_number').innerHTML = json[0].TRAIN_NUMBER;
                document.getElementById('driver_no').innerHTML = json[0].DRIVER_NO;
            }
        }
    });
...
function clickLoco(locomotiveNo) {
    ...
    document.getElementById('divButtonIconCtrl').style.display = "";
    ...
    //iChannelID  iStreamType  bZeroChannel  szRtspPort
...
function clickRealPlay1() {
    ...
    $("#playbutton2").css('background-color', 'gray');
    $("#playbutton1").css('background-color', 'blue');

    clearOPInfo();

    if (playLoco == undefined || playLoco == "" || playLoco == null) {
        showOPInfo("请选择需要直播的设备！");
        return;
    }

    //如果有正在直播的视频通道，则停止该通道的播放
    stopRealPlay();
    //获取选择设备指定端视频连接信息
...
function clickRealPlay2() {
    ...
    terminalSeq = 2;
    playobj = "(" + playLoco + "-" + terminalSeq + ")";
    $("#playbutton1").css('background-color', 'gray');
    $("#playbutton2").css('background-color', 'blue');

    clearOPInfo();

    if (playLoco == undefined || playLoco == "" || playLoco == null) {
        showOPInfo("请在左边树上选择播放的设备！");
        return;
    }
    //如果有正在直播的视频通道，则停止该通道的播放
    stopRealPlay();
...
//            playUser = json.USER_NAME
//            playIP = json.LAST_ACCESS_IP
//            isPlaying = json.IS_PLAYING
//            if (szIP == "") {
//                showOPInfo(playobj + " 未能获取到视频设备IP地址！")
//            }
//            else {
//                showOPInfo(playobj + " 获取播放参数成功！")
//                //                if (isPlaying == '0') {
//                startPlay();
//                //                }
//                //                else {
//                //                    showOPInfo(playobj + " 用户" + playUser + "正在直播，请稍后再试！")
//                //                }
//            }
//        },
//        error: function () {
...
function clickStartRecord() {
    ...
    if (oWndInfo != null) {
        ...
        if (0 == iRet) {
            szInfo = "正在录像中……";
            isRecording = true;
            document.getElementById("ImgRecord").src = "../img/player/recording.gif"
            document.getElementById("ImgRecord").title = "点击停止录像"
        } else {
            szInfo = "开始录像失败！";
        }
        showOPInfo(playobj + " " + szInfo);
    }
}
...
function clickStopRecord() {
    ...
    if (oWndInfo != null) {
        ...
        if (0 == iRet) {
            szInfo = "录像已停止！";
            isRecording = false;
            document.getElementById("ImgRecord").src = "../img/player/record.png"
            ...
            showOPInfo(playobj + " 录像文件保存在：" + recordFilePath);
