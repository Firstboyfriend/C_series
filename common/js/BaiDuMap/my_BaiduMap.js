    ...
    if (positionJson != undefined) {
        for (var i = 0; i < positionJson.length; i++) {
            ...
            if (positionJson[i].GIS_SHOW == '1') {
                ...
                var icon = new BMap.Icon("/C3/PC/MRTA/css/img/station.png", new BMap.Size(20, 30));
...
function GetSms(org, OrgType, locatype, lineCode, key, type) {
    ...
    if (SmsJson != undefined) {
        for (var i = 0; i < SmsJson.length; i++) {
            ...
            var icon;
            if (lable.split('CRH').length > 1) {
                icon = new BMap.Icon("/C3/PC/MRTA/css/img/动车.png", new BMap.Size(25, 30));
            } else {
                icon = new BMap.Icon("/C3/PC/MRTA/css/img/机车.png", new BMap.Size(25, 30));
...
function getSmsInfo(type, deviceid, startdate, enddate, txtqz, direction, startSpeed, endSpeed, txtLine) {
    ...
    if (C3SmsInfoJson != undefined) {
        ...
        if (C3SmsInfoJson[0][0].GIS_X != "" && C3SmsInfoJson[0][0].GIS_X != "0") {
            ...
            var icon = "";
            if (lable.split('CRH').length > 1) {
                icon = new BMap.Icon("/C3/PC/MRTA/css/img/动车.png", new BMap.Size(30, 37));
            } else {
                icon = new BMap.Icon("/C3/PC/MRTA/css/img/机车.png", new BMap.Size(30, 37));
